...
class ProcessMonitor(object):
    ...
    def new_process(cls, wrapper_cls, initial_options, timeout=5):
        ...
        process = multiprocessing.Process(target=start_process,
            args=(wrapper_cls, initial_options, options_queue,
                ...
                run_event))
        process.start()
        name = wrapper_cls.__class__.__name__
    ...
    def is_process_ready(self):
        return self._process_ready_event.wait(self._timeout)
        ...
        while True:
            if runtime_timeout_ms:
                ...
                if time_diff >= runtime_timeout_ms:
                    raise AssertionError('Runtime for processes timed out')
            for monitor in monitors:
...
def create_process_wrapper(f, args, kwargs, shared_options=None,
    ...
    shared_options_m = shared_options or (lambda self: {})
    setup_m = setup or (lambda self, shared_options: None)
    teardown_m = teardown or (lambda self: None)
...
class MultiprocessMethodDecorator(object):
    def __init__(self, limit):
        self._limit = limit

    def __call__(self, f):
        @wraps(f)
        def multiprocessing_method(inst, *args, **kwargs):
            klass = inst.__class__
            initial_func = getattr(klass, 'initial_options',
                    lambda inst: {})

            process_shared_options = getattr(klass, 'process_shared_options',
                    None)
            process_setup = getattr(klass, 'process_setup', None)
            process_teardown = getattr(klass, 'process_setup', None)

            initial_options = initial_func(inst)

            test_wrapper = create_process_wrapper(f, args, kwargs,
                    shared_options=process_shared_options,
                    setup=process_setup,
                    teardown=process_teardown)

            wrappers_copy = inst.wrappers[:]
            wrappers_copy.append(test_wrapper)

            manager = ProcessManager.from_wrappers(wrappers_copy,
                    initial_options, runtime_timeout=self._limit)
            manager.run()
        return multiprocessing_method


multiprocess_method = MultiprocessMethodDecorator
